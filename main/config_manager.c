...
esp_err_t config_init(system_config_t *config) {
    ...
    if (magic != MAGIC_NUMBER) {
        ESP_LOGW(TAG, "Primer inicio. Cargando defaults...");
        ...
        config->st_mode = ST_MODE_DEFAULT;
        config->log_level = 1;
    ...
    ESP_LOGI(TAG, "Configuracion encontrada. Cargando...");
    ...
    nvs_get_u32(handle, "st_mode", &config->st_mode);
